        ...
        client
          ...
          .then(
            ({ data: { exchanges, exchangeHistoricalDatas } }): PairData => {
              ...
              const tradeVolumeToken = new BigNumber(
                exchanges[0].tradeVolumeToken,
              );

              const tradeVolumeEth24HoursAgo = new BigNumber(
                exchangeHistoricalDatas[0]
                  ? exchangeHistoricalDatas[0].tradeVolumeEth
                  : 0,
              );
              const tradeVolumeToken24HoursAgo = new BigNumber(
                exchangeHistoricalDatas[0]
                  ? exchangeHistoricalDatas[0].tradeVolumeToken
                  : 0,
              );

              return {
                price,
                tradeVolumeEth: tradeVolumeEth
                  .minus(tradeVolumeEth24HoursAgo)
                  .toString(),
                tradeVolumeToken: tradeVolumeToken
                  .minus(tradeVolumeToken24HoursAgo)
                  .toString(),
              };
            },
          ),
        ...
        if (transactions.length === 0) {
          ...
          results = results.concat(
            transactions
              .map((transaction: any): Trade[] => [
                ...transaction.ethPurchaseEvents.map(
                  (event: any): Trade => ({
                    ...
                    price: new BigNumber(event.tokenAmount)
                      .dividedBy(new BigNumber(event.ethAmount))
                      .toString(),
                  }),
                ),
              ...
              .reduce((accumulator: any, trades: Trade[]): Trade[] =>
                accumulator.concat(trades),
              ),
          );
        }
      });
